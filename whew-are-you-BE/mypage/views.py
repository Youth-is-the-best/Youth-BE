...
class MyInfoAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if type_result == 'SQUIRREL':
            type_result_display = '준비성 철저한 다람쥐'
        elif type_result == 'RABBIT':
            type_result_display = '열정 가득 부지런한 토끼'
        elif type_result == 'PANDA':
            type_result_display = '재충전을 원하는 판다'
        elif type_result == 'BEAVER':
            type_result_display = '끝없는 발전을 추구하는 비버'
        elif type_result == 'EAGLE':
            type_result_display = '모험을 갈망하는 독수리'
        elif type_result == 'BEAR':
            type_result_display = '안정을 추구하는 곰'
        elif type_result == 'DOLPHIN':
            type_result_display = '호기심 많은 돌고래'
        else:
            type_result_display = '유형 테스트 결과 없음'

        return Response({
            "name": name,
            "username": username,
            "type_result": type_result,
            "type_result_display": type_result_display,
            "email": email,
            "points": points
